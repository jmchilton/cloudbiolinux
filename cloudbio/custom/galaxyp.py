...
def install_protvis(env):
    ...
    protvis_converted_files_dir = env.get('protvis_converted_files_dir', "%s/tmp/protvis" % galaxy_data_dir)
...
PATH_WHITELIST = ["%s/files/"]
CONVERTED_FILES = "%s"
''' % (env.galaxy_home, galaxy_data_dir, protvis_converted_files_dir), "%s/conf.py" % protvis_home, 0755)
